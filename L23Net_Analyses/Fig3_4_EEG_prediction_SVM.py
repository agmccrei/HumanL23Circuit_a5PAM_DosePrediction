...
df.to_csv('figs_ManualLinearRegression_V13_SVM/stats_PSD.csv')
...
def find_optimal_dose_and_range(all_data,mh,lh,uh):
	...
	for sind,s in enumerate(severities):
		for i in range(0,N_seeds):
			...
			fig.savefig('figs_ManualLinearRegression_V13_SVM/opt_dose_fits/opt_dose_fit_severity'+str(s)+'_seed'+str(i)+'.png',dpi=300,transparent=True)
			plt.close()
...
for p in range(0,NPermutes):
	...
	# train
	clf0 = svm.SVR(C=10, epsilon=0.1, kernel='linear', tol=0.001)
	clf1 = svm.SVR(C=10, epsilon=0.1, kernel='linear', tol=0.001)
	clf2 = svm.SVR(C=10, epsilon=0.1, kernel='linear', tol=0.001)
	clf3 = svm.SVR(C=10, epsilon=0.1, kernel='linear', tol=0.001)
	
	clf0.fit(np.transpose(inputs_doses_train[:-3]).tolist(), inputs_doses_train[-3])
	clf1.fit([[m] for m in inputs_doses_train[0]], inputs_doses_train[-3])
	clf2.fit([[m] for m in inputs_doses_train[1]], inputs_doses_train[-3])
	clf3.fit([[m] for m in inputs_doses_train[2]], inputs_doses_train[-3])
	
	print('SHAP output:')
	vals = ['ap_AUC', 't_AUC', 'a_AUC']
	explainer = shap.KernelExplainer(clf0.predict, data = np.transpose(inputs_doses_train[:-3]), feature_names=vals)
	...
	# test
	test0 = clf0.predict(np.transpose(inputs_doses_test[:-3]))
	test1 = clf1.predict([[m] for m in inputs_doses_test[0]])
	test2 = clf2.predict([[m] for m in inputs_doses_test[1]])
	test3 = clf3.predict([[m] for m in inputs_doses_test[2]])
	
	MSE_linear0.append(np.mean((np.array(optimal_doses_middle)-np.array(test0))**2))
	MSE_linear1.append(np.mean((np.array(optimal_doses_middle)-np.array(test1))**2))
	MSE_linear2.append(np.mean((np.array(optimal_doses_middle)-np.array(test2))**2))
	MSE_linear3.append(np.mean((np.array(optimal_doses_middle)-np.array(test3))**2))
	
	pred_dose0 = [min(doses, key=lambda x:abs(x-t)) for t in test0]
	pred_dose1 = [min(doses, key=lambda x:abs(x-t)) for t in test1]
	pred_dose2 = [min(doses, key=lambda x:abs(x-t)) for t in test2]
	pred_dose3 = [min(doses, key=lambda x:abs(x-t)) for t in test3]
	...
	# Record residuals
	test_residuals0.extend(np.abs(np.array(optimal_doses_middle)-np.array(test0)))
	test_residuals1.extend(np.abs(np.array(optimal_doses_middle)-np.array(test1)))
	test_residuals2.extend(np.abs(np.array(optimal_doses_middle)-np.array(test2)))
	test_residuals3.extend(np.abs(np.array(optimal_doses_middle)-np.array(test3)))
	
	# Record percent correct, allowing for a upper/lower limit dose error range
	tpm0 = (np.sum([ol < t < ou for ol,t,ou in zip(optimal_doses_lower,test0,optimal_doses_upper)]) / len(test0))*100
	tpm1 = (np.sum([ol < t < ou for ol,t,ou in zip(optimal_doses_lower,test1,optimal_doses_upper)]) / len(test1))*100
	tpm2 = (np.sum([ol < t < ou for ol,t,ou in zip(optimal_doses_lower,test2,optimal_doses_upper)]) / len(test2))*100
	tpm3 = (np.sum([ol < t < ou for ol,t,ou in zip(optimal_doses_lower,test3,optimal_doses_upper)]) / len(test3))*100
	...
	tpm0_a = (np.sum([t >= ou for ol,t,ou in zip(optimal_doses_lower,test0,optimal_doses_upper)]) / len(test0))*100
	tpm1_a = (np.sum([t >= ou for ol,t,ou in zip(optimal_doses_lower,test1,optimal_doses_upper)]) / len(test1))*100
	tpm2_a = (np.sum([t >= ou for ol,t,ou in zip(optimal_doses_lower,test2,optimal_doses_upper)]) / len(test2))*100
	tpm3_a = (np.sum([t >= ou for ol,t,ou in zip(optimal_doses_lower,test3,optimal_doses_upper)]) / len(test3))*100
	
	tpm0_b = (np.sum([t <= ol for ol,t,ou in zip(optimal_doses_lower,test0,optimal_doses_upper)]) / len(test0))*100
	tpm1_b = (np.sum([t <= ol for ol,t,ou in zip(optimal_doses_lower,test1,optimal_doses_upper)]) / len(test1))*100
	tpm2_b = (np.sum([t <= ol for ol,t,ou in zip(optimal_doses_lower,test2,optimal_doses_upper)]) / len(test2))*100
	tpm3_b = (np.sum([t <= ol for ol,t,ou in zip(optimal_doses_lower,test3,optimal_doses_upper)]) / len(test3))*100
...
#clf3 = svm.SVR(C=10, epsilon=0.1, kernel='linear', tol=0.001)
#
#y_lin = clf0.fit(np.transpose(inputs).tolist(), optimal_dose).predict(np.transpose(inputs).tolist())
#y_lin0 = clf1.fit([[m] for m in inputs[0]], optimal_dose).predict([[m] for m in inputs[0]])
#y_lin1 = clf2.fit([[m] for m in inputs[1]], optimal_dose).predict([[m] for m in inputs[1]])
#y_lin2 = clf3.fit([[m] for m in inputs[2]], optimal_dose).predict([[m] for m in inputs[2]])

y_lin = best_model0.predict(np.transpose(inputs).tolist())
y_lin0 = best_model1.predict([[m] for m in inputs[0]])
y_lin1 = best_model2.predict([[m] for m in inputs[1]])
y_lin2 = best_model3.predict([[m] for m in inputs[2]])

print('Best model SHAP output:')
vals = ['ap_AUC', 't_AUC', 'a_AUC']
print(vals)
explainer = shap.KernelExplainer(best_model0.predict, data = np.transpose(inputs), feature_names=vals)
shap_values = explainer(np.transpose(inputs))
mean_shaps = abs(shap_values.values).mean(axis=0)
print(mean_shaps)
...
fig_shaps.savefig('figs_ManualLinearRegression_V13_SVM/shaps.png',dpi=300)
plt.close()
...
fig_dosefits.savefig('figs_ManualLinearRegression_V13_SVM/scatters_severity_optimaldose_withOverallFit.png',dpi=300,transparent=True)
plt.close()
...
fig.tight_layout()
fig.savefig('figs_ManualLinearRegression_V13_SVM/scatters_severity_optimaldose_SVRlinear_withOverallFit.png',dpi=300,transparent=True)
plt.close()

min_overall = np.min([np.min(MSE_linear0),np.min(MSE_linear1),np.min(MSE_linear2),np.min(MSE_linear3)])
max_overall = np.max([np.max(MSE_linear0),np.max(MSE_linear1),np.max(MSE_linear2),np.max(MSE_linear3)])
bins_sequence = np.linspace(min_overall,max_overall,50)

fig_MSE, ax_MSES = plt.subplots(1,1, figsize=(16,5))
ax_MSES.hist(MSE_linear0,range=(min_overall,max_overall),bins=bins_sequence,color='tab:orange',label='MV',alpha=0.4)
ax_MSES.hist(MSE_linear1,range=(min_overall,max_overall),bins=bins_sequence,color='tab:green',label='1/f',alpha=0.4)
ax_MSES.hist(MSE_linear2,range=(min_overall,max_overall),bins=bins_sequence,color='tab:blue',label=r'$\theta$',alpha=0.4)
ax_MSES.hist(MSE_linear3,range=(min_overall,max_overall),bins=bins_sequence,color='tab:purple',label=r'$\alpha$',alpha=0.4)
ax_MSES.set_xlabel('Tested Mean Squared Error')
ax_MSES.set_ylabel('Count')
ax_MSES.legend(fontsize=20)
fig_MSE.tight_layout()
fig_MSE.savefig('figs_ManualLinearRegression_V13_SVM/MSE_multi_linear_model.png',dpi=300,transparent=True)
plt.close()

mean_percent_correct = [np.mean(test_percent_correct0),np.mean(test_percent_correct1),np.mean(test_percent_correct2),np.mean(test_percent_correct3)]
sd_percent_correct = [np.std(test_percent_correct0),np.std(test_percent_correct1),np.std(test_percent_correct2),np.std(test_percent_correct3)]
...
fig_percentcorrect.savefig('figs_ManualLinearRegression_V13_SVM/percentcorrect.png',dpi=300)
plt.close()

mean_residuals = [np.mean(test_residuals0),np.mean(test_residuals1),np.mean(test_residuals2),np.mean(test_residuals3)]
sd_residuals = [np.std(test_residuals0),np.std(test_residuals1),np.std(test_residuals2),np.std(test_residuals3)]
...
fig_residuals.savefig('figs_ManualLinearRegression_V13_SVM/residuals.png',dpi=300)
plt.close()

# Save accuracies to npy file for stats later
pmDose_all = [test_pmDose_percent_correct0,test_pmDose_percent_correct1,test_pmDose_percent_correct2,test_pmDose_percent_correct3]
np.save('figs_ManualLinearRegression_V13_SVM/accuracy_pmDose_allvalues.npy',pmDose_all)
pmDose_all_above = [test_pmDose_percent_incorrect0_above,test_pmDose_percent_incorrect1_above,test_pmDose_percent_incorrect2_above,test_pmDose_percent_incorrect3_above]
np.save('figs_ManualLinearRegression_V13_SVM/accuracy_pmDose_allvalues_above.npy',pmDose_all_above)
pmDose_all_below = [test_pmDose_percent_incorrect0_below,test_pmDose_percent_incorrect1_below,test_pmDose_percent_incorrect2_below,test_pmDose_percent_incorrect3_below]
np.save('figs_ManualLinearRegression_V13_SVM/accuracy_pmDose_allvalues_below.npy',pmDose_all_below)

mean_pmDose_percent_correct = [np.mean(test_pmDose_percent_correct0),np.mean(test_pmDose_percent_correct1),np.mean(test_pmDose_percent_correct2),np.mean(test_pmDose_percent_correct3)]
sd_pmDose_percent_correct = [np.std(test_pmDose_percent_correct0),np.std(test_pmDose_percent_correct1),np.std(test_pmDose_percent_correct2),np.std(test_pmDose_percent_correct3)]
xvals = np.linspace(1,len(mean_pmDose_percent_correct),len(mean_pmDose_percent_correct))

mean_pmDose_percent_incorrect_above = [np.mean(test_pmDose_percent_incorrect0_above),np.mean(test_pmDose_percent_incorrect1_above),np.mean(test_pmDose_percent_incorrect2_above),np.mean(test_pmDose_percent_incorrect3_above)]
sd_pmDose_percent_incorrect_above = [np.std(test_pmDose_percent_incorrect0_above),np.std(test_pmDose_percent_incorrect1_above),np.std(test_pmDose_percent_incorrect2_above),np.std(test_pmDose_percent_incorrect3_above)]
xvals = np.linspace(1,len(mean_pmDose_percent_incorrect_above),len(mean_pmDose_percent_incorrect_above))

mean_pmDose_percent_incorrect_below = [np.mean(test_pmDose_percent_incorrect0_below),np.mean(test_pmDose_percent_incorrect1_below),np.mean(test_pmDose_percent_incorrect2_below),np.mean(test_pmDose_percent_incorrect3_below)]
sd_pmDose_percent_incorrect_below = [np.std(test_pmDose_percent_incorrect0_below),np.std(test_pmDose_percent_incorrect1_below),np.std(test_pmDose_percent_incorrect2_below),np.std(test_pmDose_percent_incorrect3_below)]
...
fig_percentcorrect.patch.set_alpha(0)
fig_percentcorrect.tight_layout()
fig_percentcorrect.savefig('figs_ManualLinearRegression_V13_SVM/percentcorrect_pmDose_withabovebelow.png',dpi=300)
plt.close()

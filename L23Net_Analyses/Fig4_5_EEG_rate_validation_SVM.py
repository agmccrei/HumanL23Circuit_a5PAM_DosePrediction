...
post_fit = np.load('figs_ManualLinearRegression_PSDvalidation_V5_SVM/post_stimulus_default_circuit.npy')
...
for p in range(0,NPermutes):
	...
	clf4 = svm.SVR(C=10, epsilon=0.1, kernel='linear', tol=0.001)
	...
	clf4.fit([[m] for m in inputs_doses_train[3]], inputs_doses_train[-6])
	...
	test4 = clf4.predict([[m] for m in inputs_doses_test[3]])
	...
	tpm4 = (np.sum([ol < t < ou for ol,t,ou in zip(optimal_doses_lower_r,test4,optimal_doses_upper_r)]) / len(test4))*100
...
y_lin = best_model0.predict(np.transpose(best_test0[:3]).tolist())
y_lin0 = best_model1.predict([[m] for m in best_test1[0]])
y_lin1 = best_model2.predict([[m] for m in best_test2[1]])
y_lin2 = best_model3.predict([[m] for m in best_test3[2]])
y_lin3 = best_model4.predict([[m] for m in best_test4[3]])

# Replace negative predictions with 0
test = [t if t>=0 else 0 for t in y_lin]
test_linear0 = [t if t>=0 else 0 for t in y_lin0]
test_linear1 = [t if t>=0 else 0 for t in y_lin1]
test_linear2 = [t if t>=0 else 0 for t in y_lin2]
test_linear3 = [t if t>=0 else 0 for t in y_lin3]
...
fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/scatters_severity_optimaldose_withOverallFit.png',dpi=300,transparent=True)
plt.close()
...
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/opt_dose_fit_severity_3D_performance_MVpredicted.png',dpi=300,transparent=True)
plt.close()
...
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/opt_dose_fit_severity_3D_biomarkers_MVpredicted.png',dpi=300,transparent=True)
plt.close()
...
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/opt_dose_fit_severity_3D_biomarkers_RATEpredicted.png',dpi=300,transparent=True)
plt.close()
...
df.to_csv('figs_ManualLinearRegression_PSDvalidation_V5_SVM/stats_predicted_dose_vs_healthy.csv')
...
df.to_csv('figs_ManualLinearRegression_PSDvalidation_V5_SVM/stats_rate_predicted_dose_vs_healthy.csv')
...
def plot_validation(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename):
	...
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/'+filename+'.png',dpi=300,transparent=True)
...
def plot_validation_mean_CI(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename):
	...
	ax_dosefits.set_ylabel(ylabel)
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/mean_ci_'+filename+'.png',dpi=300,transparent=True)
	plt.close(fig_dosefits)
...
def plot_validation_mean_CI_only(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename):
	...
	ax_dosefits.set_ylabel(ylabel)
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/mean_ci_only_'+filename+'.png',dpi=300,transparent=True)
	plt.close(fig_dosefits)
...
def plot_validation_mean_CI_only_split(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename,fs):
	...
	ax_dosefits.set_ylabel(ylabel)
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5_SVM/mean_ci_only_split_'+filename+'.png',dpi=300,transparent=True)
	plt.close(fig_dosefits)

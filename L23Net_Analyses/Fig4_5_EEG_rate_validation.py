...
PLOT_dose_response_fits = False # Much slower if True
...
post_fit = np.load('figs_ManualLinearRegression_PSDvalidation_V5/post_stimulus_default_circuit.npy')
...
def run_multi_line_fits(data,opt_doses,opt_doses_rate):
	# General Equation:
	...
	s4 = (np.max(opt_doses_rate)-np.min(opt_doses_rate))/(np.mean(data[3][(opt_doses_rate==np.max(opt_doses_rate))])-np.mean(data[3][(opt_doses_rate==np.min(opt_doses_rate))]))
	...
	i4 = np.mean([od - s4*d4 for od,d4 in zip(opt_doses_rate,data[3])])
	...
	coeff_l_4 = curve_fit(linear, data[3], opt_doses_rate, p0 = p4, full_output=True)
	...
	ax_dosefits[3].scatter(data[3], opt_doses_rate, c='k')
...
for p in range(0,NPermutes):
	...
	# train
	coeffs, fig_dosefits, coeffs_linear0, coeffs_linear1, coeffs_linear2, coeffs_linear3 = run_multi_line_fits(inputs_doses_train[:-3],inputs_doses_train[-3],inputs_doses_train[-6])
	...
	tpm3 = (np.sum([ol < t < ou for ol,t,ou in zip(optimal_doses_lower_r,test_linear3,optimal_doses_upper_r)]) / len(test_linear3))*100
	
	# Save best model across permutations
	if tpm > best_tpm:
		best_model = coeffs
		best_train = inputs_doses_train
		best_test = inputs_doses_test
		best_tpm = tpm
	if tpm0 > best_tpm0:
...
#s4 = (np.max(optimal_dose_rate)-np.min(optimal_dose_rate))/(np.mean(data[3][(optimal_dose_rate==np.max(optimal_dose_rate))])-np.mean(data[3][(optimal_dose_rate==np.min(optimal_dose_rate))]))
#
#int0 = np.mean([od - s1*d1 - s2*d2 - s3*d3 for od,d1,d2,d3 in zip(optimal_dose,data[0],data[1],data[2])])
...
#int4 = np.mean([od - s4*d4 for od,d4 in zip(optimal_dose_rate,data[3])])
#
#p0_l = [int0, s1, s2, s3]
#p1 = [s1, int1]
#p2 = [s2, int2]
...
#coeff_l_4 = curve_fit(linear, data[3], optimal_dose_rate, p0 = p4, full_output=True)
...
print('Dose = '+f"{coeff_l_4[0][0]:0.2f}"+'rate + '+f"{coeff_l_4[0][1]:0.2f}")
...
fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5/scatters_severity_optimaldose_withOverallFit.png',dpi=300,transparent=True)
plt.close()

# Get predicted doses from overall fits
test = multi_linear(np.array(best_test[:3]), *coeff_l[0])
test_linear0 = linear(np.array(best_test0[0]), *coeff_l_1[0])
test_linear1 = linear(np.array(best_test1[1]), *coeff_l_2[0])
test_linear2 = linear(np.array(best_test2[2]), *coeff_l_3[0])
test_linear3 = linear(np.array(best_test3[3]), *coeff_l_4[0])

# Replace negative predictions with 0
test = [t if t>=0 else 0 for t in test]
test_linear0 = [t if t>=0 else 0 for t in test_linear0]
test_linear1 = [t if t>=0 else 0 for t in test_linear1]
test_linear2 = [t if t>=0 else 0 for t in test_linear2]
test_linear3 = [t if t>=0 else 0 for t in test_linear3]
...
#
#pred_dose_ind = [min(enumerate(doses), key=lambda x: abs(x[1]-t))[0] for t in test]
#pred_dose0_ind = [min(enumerate(doses), key=lambda x: abs(x[1]-t))[0] for t in test_linear0]
#pred_dose1_ind = [min(enumerate(doses), key=lambda x: abs(x[1]-t))[0] for t in test_linear1]
#pred_dose2_ind = [min(enumerate(doses), key=lambda x: abs(x[1]-t))[0] for t in test_linear2]
#pred_dose3_ind = [min(enumerate(doses), key=lambda x: abs(x[1]-t))[0] for t in test_linear3]

# find rate, failed, false at 0 dose conditions and pair them with the values at the predicted dose condition
# indexing remind: metric[doses][severities][seeds]
# inputs variable alredy contains 0 dose condition values and is organized [:20] for 0.1 severity, [20:40] for 0.2 severity, and [40:60] for 0.3 severity, and [60:] for 0.4 severity
...
def plot_dose_response_fits(cind2_, seedind_, coeff_rate, coeff_failed, coeff_false, pd, filename):
	data_doses_rate =[x[cind2_][seedind_] for x in base_rates]
	data_doses_failed =[x[cind2_][seedind_] for x in failed_detections]
	data_doses_false =[x[cind2_][seedind_] for x in false_detections]
	
	xvals_highres_d = np.linspace(doses[0],doses[-1],10**3)
	l_fit_rate = linear(xvals_highres_d, *coeff_rate[0])
	l_fit_failed = expdec(xvals_highres_d, *coeff_failed[0])
	l_fit_false = expdec(xvals_highres_d, *coeff_false[0])
	
	od_fit_rate = linear(pd, *coeff_rate[0])
	od_fit_failed = expdec(pd, *coeff_failed[0])
	od_fit_false = expdec(pd, *coeff_false[0])
	
	fig_rate, ax_rate = plt.subplots(figsize=(6, 5))
	fig_failed, ax_failed = plt.subplots(figsize=(6, 5))
	fig_false, ax_false = plt.subplots(figsize=(6, 5))
	
	ax_rate.plot(xvals_highres_d*100,l_fit_rate, c='k', zorder=1)
	ax_failed.plot(xvals_highres_d*100,l_fit_failed, c='k', zorder=1)
	ax_false.plot(xvals_highres_d*100,l_fit_false, c='k', zorder=1)
	
	ax_rate.scatter([d*100 for d in doses], data_doses_rate, s=7**2, marker='o', c='k', zorder=3)
	ax_failed.scatter([d*100 for d in doses], data_doses_failed, s=7**2, marker='o', c='k', zorder=3)
	ax_false.scatter([d*100 for d in doses], data_doses_false, s=7**2, marker='o', c='k', zorder=3)
	
	lower_healthy_rate = base_rates_lu[0][0][0]
	upper_healthy_rate = base_rates_lu[0][0][1]
	lower_healthy_failed = failed_detections_lu[0][0][0]
	upper_healthy_failed = failed_detections_lu[0][0][1]
	lower_healthy_false = false_detections_lu[0][0][0]
	upper_healthy_false = false_detections_lu[0][0][1]
	
	#ax_rate.plot([doses[0], doses[-1]],[base_rates_m[0],base_rates_m[0]],'k',ls='dashed',alpha=1)
	#ax_failed.plot([doses[0], doses[-1]],[failed_detections_m[0],failed_detections_m[0]],'k',ls='dashed',alpha=1)
	#ax_false.plot([doses[0], doses[-1]],[false_detections_m[0],false_detections_m[0]],'k',ls='dashed',alpha=1)
	
	#ax_rate.plot([pd*100, pd*100],[np.min(data_doses_rate),np.max(data_doses_rate)],'tab:blue',ls='dashed',alpha=1)
	#ax_failed.plot([pd*100, pd*100],[np.min(data_doses_failed),np.max(data_doses_failed)],'tab:blue',ls='dashed',alpha=1)
	#ax_false.plot([pd*100, pd*100],[np.min(data_doses_false),np.max(data_doses_false)],'tab:blue',ls='dashed',alpha=1)
	ax_rate.scatter(pd*100,od_fit_rate, s=10**2, marker='o',c='tab:blue', zorder=4)
	ax_failed.scatter(pd*100,od_fit_failed, s=10**2, marker='o',c='tab:blue', zorder=4)
	ax_false.scatter(pd*100,od_fit_false, s=10**2, marker='o',c='tab:blue', zorder=4)
	
	ax_rate.fill_between([doses[0]*100,doses[-1]*100],y1=[upper_healthy_rate,upper_healthy_rate],y2=[lower_healthy_rate,lower_healthy_rate],color='k', alpha=0.2, zorder=2)
	ax_failed.fill_between([doses[0]*100,doses[-1]*100],y1=[upper_healthy_failed,upper_healthy_failed],y2=[lower_healthy_failed,lower_healthy_failed],color='k', alpha=0.2, zorder=2)
	ax_false.fill_between([doses[0]*100,doses[-1]*100],y1=[upper_healthy_false,upper_healthy_false],y2=[lower_healthy_false,lower_healthy_false],color='k', alpha=0.2, zorder=2)
	
	ax_rate.set_xticks([d*100 for i,d in enumerate(doses) if i % 2 == 0])
	ax_rate.set_xlabel('% of Reference Dose')
	ax_rate.set_ylabel('Rate (Hz)')
	ax_rate.set_xlim((doses[0]*100-0.02, doses[-1]*100+0.02))
	ax_rate.set_ylim((np.min(data_doses_rate),np.max(data_doses_rate)))
	fig_rate.tight_layout()
	fig_rate.savefig('figs_ManualLinearRegression_PSDvalidation_V5/_linearfits_individual_seeds/'+filename+'rate_seed_'+str(seedind_)+'_severity_'+str(severities[cind2_])+'.png',dpi=300,transparent=True)
	plt.close(fig_rate)
	
	ax_failed.set_xticks([d*100 for i,d in enumerate(doses) if i % 2 == 0])
	ax_failed.set_xlabel('% of Reference Dose')
	ax_failed.set_ylabel('Failed Detections (%)')
	ax_failed.set_xlim((doses[0]*100-0.02, doses[-1]*100+0.02))
	ax_failed.set_ylim((np.min(data_doses_failed),np.max(data_doses_failed)))
	fig_failed.tight_layout()
	fig_failed.savefig('figs_ManualLinearRegression_PSDvalidation_V5/_linearfits_individual_seeds/'+filename+'failed_seed_'+str(seedind_)+'_severity_'+str(severities[cind2_])+'.png',dpi=300,transparent=True)
	plt.close(fig_failed)
	
	ax_false.set_xticks([d*100 for i,d in enumerate(doses) if i % 2 == 0])
	ax_false.set_xlabel('% of Reference Dose')
	ax_false.set_ylabel('False Detections (%)')
	ax_false.set_xlim((doses[0]*100-0.02, doses[-1]*100+0.02))
	ax_false.set_ylim((np.min(data_doses_false),np.max(data_doses_false)))
	fig_false.tight_layout()
	fig_false.savefig('figs_ManualLinearRegression_PSDvalidation_V5/_linearfits_individual_seeds/'+filename+'false_seed_'+str(seedind_)+'_severity_'+str(severities[cind2_])+'.png',dpi=300,transparent=True)
	plt.close(fig_false)

def populate_prediction_data(predicted_doses,optimal_dose,sevs,seeds,filename):
	...
	for ind, predicted_dose in enumerate(predicted_doses):
		...
		if cind2 == 0:
			...
			truedose_r = linear(optimal_dose[ind],*coeff_d_rate[0])
			zerodose_fai = failed_detections[0][cind2][seedind]
			preddose_fai = expdec(predicted_doses[ind],*coeff_d_failed[0])
			truedose_fai = expdec(optimal_dose[ind],*coeff_d_failed[0])
			zerodose_fal = false_detections[0][cind2][seedind]
			preddose_fal = expdec(predicted_doses[ind],*coeff_d_false[0])
			truedose_fal = expdec(optimal_dose[ind],*coeff_d_false[0])
			if PLOT_dose_response_fits: plot_dose_response_fits(cind2, seedind, coeff_d_rate, coeff_d_failed, coeff_d_false, predicted_doses[ind],filename)
			
			rate_predictor_00sst.append([zerodose_r,preddose_r,truedose_r])
			failed_predictor_00sst.append([zerodose_fai,preddose_fai,truedose_fai])
			false_predictor_00sst.append([zerodose_fal,preddose_fal,truedose_fal])
		elif cind2 == 1:
			coeff_d_rate, coeff_d_failed, coeff_d_false = get_dose_response_fits(cind2,seedind)
			zerodose_r = base_rates[0][cind2][seedind]
			preddose_r = linear(predicted_doses[ind],*coeff_d_rate[0])
			...
			rate_predictor_10sst.append([zerodose_r,preddose_r,truedose_r])
			failed_predictor_10sst.append([zerodose_fai,preddose_fai,truedose_fai])
			false_predictor_10sst.append([zerodose_fal,preddose_fal,truedose_fal])
		elif cind2 == 2:
			coeff_d_rate, coeff_d_failed, coeff_d_false = get_dose_response_fits(cind2,seedind)
			zerodose_r = base_rates[0][cind2][seedind]
			preddose_r = linear(predicted_doses[ind],*coeff_d_rate[0])
			...
			rate_predictor_20sst.append([zerodose_r,preddose_r,truedose_r])
			failed_predictor_20sst.append([zerodose_fai,preddose_fai,truedose_fai])
			false_predictor_20sst.append([zerodose_fal,preddose_fal,truedose_fal])
		elif cind2 == 3:
			coeff_d_rate, coeff_d_failed, coeff_d_false = get_dose_response_fits(cind2,seedind)
			zerodose_r = base_rates[0][cind2][seedind]
			preddose_r = linear(predicted_doses[ind],*coeff_d_rate[0])
			...
			rate_predictor_30sst.append([zerodose_r,preddose_r,truedose_r])
			failed_predictor_30sst.append([zerodose_fai,preddose_fai,truedose_fai])
			false_predictor_30sst.append([zerodose_fal,preddose_fal,truedose_fal])
		elif cind2 == 4:
			coeff_d_rate, coeff_d_failed, coeff_d_false = get_dose_response_fits(cind2,seedind)
			zerodose_r = base_rates[0][cind2][seedind]
			preddose_r = linear(predicted_doses[ind],*coeff_d_rate[0])
			...
			rate_predictor_40sst.append([zerodose_r,preddose_r,truedose_r])
			failed_predictor_40sst.append([zerodose_fai,preddose_fai,truedose_fai])
			false_predictor_40sst.append([zerodose_fal,preddose_fal,truedose_fal])
...
rate_mv_00sst,rate_mv_10sst,rate_mv_20sst,rate_mv_30sst,rate_mv_40sst,failed_mv_00sst,failed_mv_10sst,failed_mv_20sst,failed_mv_30sst,failed_mv_40sst,false_mv_00sst,false_mv_10sst,false_mv_20sst,false_mv_30sst,false_mv_40sst = populate_prediction_data(test,best_test[-3],best_test[4],best_test[5],'predicted_by_MV_')
rate_aperiodic_00sst,rate_aperiodic_10sst,rate_aperiodic_20sst,rate_aperiodic_30sst,rate_aperiodic_40sst,failed_aperiodic_00sst,failed_aperiodic_10sst,failed_aperiodic_20sst,failed_aperiodic_30sst,failed_aperiodic_40sst,false_aperiodic_00sst,false_aperiodic_10sst,false_aperiodic_20sst,false_aperiodic_30sst,false_aperiodic_40sst = populate_prediction_data(test_linear0,best_test0[-3],best_test0[4],best_test0[5],'predicted_by_aperiodic_')
rate_theta_00sst,rate_theta_10sst,rate_theta_20sst,rate_theta_30sst,rate_theta_40sst,failed_theta_00sst,failed_theta_10sst,failed_theta_20sst,failed_theta_30sst,failed_theta_40sst,false_theta_00sst,false_theta_10sst,false_theta_20sst,false_theta_30sst,false_theta_40sst = populate_prediction_data(test_linear1,best_test1[-3],best_test1[4],best_test1[5],'predicted_by_theta_')
rate_alpha_00sst,rate_alpha_10sst,rate_alpha_20sst,rate_alpha_30sst,rate_alpha_40sst,failed_alpha_00sst,failed_alpha_10sst,failed_alpha_20sst,failed_alpha_30sst,failed_alpha_40sst,false_alpha_00sst,false_alpha_10sst,false_alpha_20sst,false_alpha_30sst,false_alpha_40sst = populate_prediction_data(test_linear2,best_test2[-3],best_test2[4],best_test2[5],'predicted_by_alpha_')
rate_rate_00sst,rate_rate_10sst,rate_rate_20sst,rate_rate_30sst,rate_rate_40sst,failed_rate_00sst,failed_rate_10sst,failed_rate_20sst,failed_rate_30sst,failed_rate_40sst,false_rate_00sst,false_rate_10sst,false_rate_20sst,false_rate_30sst,false_rate_40sst = populate_prediction_data(test_linear3,best_test3[-6],best_test3[4],best_test3[5],'predicted_by_rate_')
...
def populate_prediction_data_EEG(predicted_doses,optimal_dose,sevs,seeds):
	...
	for ind, predicted_dose in enumerate(predicted_doses):
		...
		if cind2 == 0:
			...
			truedose_ap = linear(optimal_dose[ind],*coeff_d_aperiodic[0])
			zerodose_th = AUC_t_abs[0][cind2][seedind]
			preddose_th = linear(predicted_doses[ind],*coeff_d_theta[0])
			truedose_th = linear(optimal_dose[ind],*coeff_d_theta[0])
			zerodose_al = AUC_a_abs[0][cind2][seedind]
			preddose_al = linear(predicted_doses[ind],*coeff_d_alpha[0])
			truedose_al = linear(optimal_dose[ind],*coeff_d_alpha[0])

			aperiodic_predictor_00sst.append([zerodose_ap,preddose_ap,truedose_ap])
			theta_predictor_00sst.append([zerodose_th,preddose_th,truedose_th])
			alpha_predictor_00sst.append([zerodose_al,preddose_al,truedose_al])
		elif cind2 == 1:
			coeff_d_aperiodic, coeff_d_theta, coeff_d_alpha = get_dose_response_fits_EEG(cind2,seedind)
			zerodose_ap = AUC[0][cind2][seedind]
			preddose_ap = linear(predicted_doses[ind],*coeff_d_aperiodic[0])
			...
			aperiodic_predictor_10sst.append([zerodose_ap,preddose_ap,truedose_ap])
			theta_predictor_10sst.append([zerodose_th,preddose_th,truedose_th])
			alpha_predictor_10sst.append([zerodose_al,preddose_al,truedose_al])
		elif cind2 == 2:
			coeff_d_aperiodic, coeff_d_theta, coeff_d_alpha = get_dose_response_fits_EEG(cind2,seedind)
			zerodose_ap = AUC[0][cind2][seedind]
			preddose_ap = linear(predicted_doses[ind],*coeff_d_aperiodic[0])
			...
			aperiodic_predictor_20sst.append([zerodose_ap,preddose_ap,truedose_ap])
			theta_predictor_20sst.append([zerodose_th,preddose_th,truedose_th])
			alpha_predictor_20sst.append([zerodose_al,preddose_al,truedose_al])
		elif cind2 == 3:
			coeff_d_aperiodic, coeff_d_theta, coeff_d_alpha = get_dose_response_fits_EEG(cind2,seedind)
			zerodose_ap = AUC[0][cind2][seedind]
			preddose_ap = linear(predicted_doses[ind],*coeff_d_aperiodic[0])
			...
			aperiodic_predictor_30sst.append([zerodose_ap,preddose_ap,truedose_ap])
			theta_predictor_30sst.append([zerodose_th,preddose_th,truedose_th])
			alpha_predictor_30sst.append([zerodose_al,preddose_al,truedose_al])
		elif cind2 == 4:
			coeff_d_aperiodic, coeff_d_theta, coeff_d_alpha = get_dose_response_fits_EEG(cind2,seedind)
			zerodose_ap = AUC[0][cind2][seedind]
			preddose_ap = linear(predicted_doses[ind],*coeff_d_aperiodic[0])
			...
			aperiodic_predictor_40sst.append([zerodose_ap,preddose_ap,truedose_ap])
			theta_predictor_40sst.append([zerodose_th,preddose_th,truedose_th])
			alpha_predictor_40sst.append([zerodose_al,preddose_al,truedose_al])
...
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5/opt_dose_fit_severity_3D_performance_MVpredicted.png',dpi=300,transparent=True)
for m1,m2,m3 in zip(combined_false_mv,combined_false_mv,combined_rate_mv):
	ax.scatter(m1[2], m2[2], m3[2], c='white', s=14**2, edgecolors='limegreen', linewidth=2) # Plot magenta color to outline zero dose
fig.tight_layout()
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5/opt_dose_fit_severity_3D_performance_MVpredicted_withTrueDoses.png',dpi=300,transparent=True)
...
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5/opt_dose_fit_severity_3D_biomarkers_MVpredicted.png',dpi=300,transparent=True)
for m1,m2,m3 in zip(combined_aperiodic_mv,combined_theta_mv,combined_alpha_mv):
	ax.scatter(m1[2], m2[2], m3[2], c='limegreen', s=14**2, edgecolors='k', linewidth=2) # Plot magenta color to outline zero dose
fig.tight_layout()
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5/opt_dose_fit_severity_3D_biomarkers_MVpredicted_withTrueDoses.png',dpi=300,transparent=True)
...
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5/opt_dose_fit_severity_3D_biomarkers_RATEpredicted.png',dpi=300,transparent=True)
for m1,m2,m3 in zip(combined_aperiodic_rate,combined_theta_rate,combined_alpha_rate):
	ax.scatter(m1[2], m2[2], m3[2], c='limegreen', s=14**2, edgecolors='k', linewidth=2) # Plot magenta color to outline zero dose
fig.tight_layout()
fig.savefig('figs_ManualLinearRegression_PSDvalidation_V5/opt_dose_fit_severity_3D_biomarkers_RATEpredicted_withTrueDoses.png',dpi=300,transparent=True)

plt.close()

# Stats compared to healthy (first convert combined arrays to only contain predicted)
...
df.to_csv('figs_ManualLinearRegression_PSDvalidation_V5/stats_predicted_dose_vs_healthy.csv')
...
df.to_csv('figs_ManualLinearRegression_PSDvalidation_V5/stats_rate_predicted_dose_vs_healthy.csv')
...
def plot_validation(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename):
	...
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5/'+filename+'.png',dpi=300,transparent=True)
...
def plot_validation_mean_CI(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename):
	...
	for m1 in metric_10sst:
		ax_dosefits.plot([1,2],m1[:2],color='royalblue',linewidth=2,alpha=0.1)
		if healthy_range[0] < m1[1] < healthy_range[1]: count_recovered+=1
	for m2 in metric_20sst:
		ax_dosefits.plot([1,2],m2[:2],color='royalblue',linewidth=2,alpha=0.1)
		if healthy_range[0] < m2[1] < healthy_range[1]: count_recovered+=1
	for m3 in metric_30sst:
		ax_dosefits.plot([1,2],m3[:2],color='darkviolet',linewidth=2,alpha=0.1)
		if healthy_range[0] < m3[1] < healthy_range[1]: count_recovered+=1
	for m4 in metric_40sst:
		ax_dosefits.plot([1,2],m4[:2],color='deeppink',linewidth=2,alpha=0.1)
		if healthy_range[0] < m4[1] < healthy_range[1]: count_recovered+=1
	ax_dosefits.scatter([1,2],combined_m[:2],s=10**2,color='k')
	ax_dosefits.errorbar([1,2],combined_m[:2],yerr=combined_err[:2],color='k',capsize=6,capthick=3,elinewidth=3, linestyle='')
	percent_recovered = (count_recovered/(len(metric_10sst)+len(metric_20sst)+len(metric_30sst)+len(metric_40sst)))*100
	y_loc = np.mean(np.transpose(metric_40sst)[0])
	ax_dosefits.text(2,y_loc,f"{percent_recovered:0.0f}"+'%',ha='center')
	ax_dosefits.set_xlim(0.7,2.3)
	...
	ax_dosefits.set_ylabel(ylabel)
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5/mean_ci_'+filename+'.png',dpi=300,transparent=True)
	plt.close(fig_dosefits)
...
def plot_validation_mean_CI_only(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename):
	...
	ax_dosefits.set_ylabel(ylabel)
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5/mean_ci_only_'+filename+'.png',dpi=300,transparent=True)
	plt.close(fig_dosefits)
...
def plot_validation_mean_CI_only_split(metric_10sst,metric_20sst,metric_30sst,metric_40sst,healthy_range,ylabel,filename,fs):
	...
	combined_m = np.mean(combined,axis=0)[:2]
	metric_sst10_m = np.mean(metric_10sst,axis=0)[:2]
	metric_sst20_m = np.mean(metric_20sst,axis=0)[:2]
	metric_sst30_m = np.mean(metric_30sst,axis=0)[:2]
	metric_sst40_m = np.mean(metric_40sst,axis=0)[:2]
	combined_err = np.std(combined,axis=0)[:2]
	metric_sst10_err = np.std(metric_10sst,axis=0)[:2]
	metric_sst20_err = np.std(metric_20sst,axis=0)[:2]
	metric_sst30_err = np.std(metric_30sst,axis=0)[:2]
	metric_sst40_err = np.std(metric_40sst,axis=0)[:2]
	...
	ax_dosefits.set_ylabel(ylabel)
	fig_dosefits.tight_layout()
	fig_dosefits.savefig('figs_ManualLinearRegression_PSDvalidation_V5/mean_ci_only_split_'+filename+'.png',dpi=300,transparent=True)
	plt.close(fig_dosefits)
...
plot_validation_mean_CI_only_split(rate_rate_10sst,rate_rate_20sst,rate_rate_30sst,rate_rate_40sst,base_rates_lu[0][0],'Spike Rate (Hz)','validation_rate_RatePredicted',(5,4.5))#(3.4,4.0))
...
plot_validation_mean_CI_only_split(failed_rate_10sst,failed_rate_20sst,failed_rate_30sst,failed_rate_40sst,failed_detections_lu[0][0],'Failed (%)','validation_failed_RatePredicted',(5,4.5))#(3.4,4.0))
...
plot_validation_mean_CI_only_split(false_rate_10sst,false_rate_20sst,false_rate_30sst,false_rate_40sst,false_detections_lu[0][0],'False (%)','validation_false_RatePredicted',(5,4.5))#(3.4,4.0))

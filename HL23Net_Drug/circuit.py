...
#===========================================================================
TESTING = False # i.e. generate 1 cell/pop, with 0.1 s runtime
no_connectivity = False

stimulate = False # Add a stimulus
MDD = 0.1 # decrease SST tonic & synaptic inhibition by 40% (i.e. 0.4 is reference MDD)
DRUG_a5PAM = 0.25 # value is a dose multiplier factor (0 - 1.5, 1 as the reference dose) - ignored if 0
...
if MDD>0:
	...
	for pre in cell_names:
		for post in cell_names:
			if 'SST' in pre:
				circuit_params["syn_params"][pre+post]["gmax"] = circuit_params["syn_cond"].at[pre, post]*(1-MDD) # Synaptic reduction
	# tonic reduction
	for post in cell_names:
		if 'PYR' in post:
			circuit_params['SING_CELL_PARAM'].at['apic_tonic',post] = circuit_params['SING_CELL_PARAM'].at['apic_tonic',post]*(1-MDD)
		else:
			sst = 0
			total = 0
			for pre in cell_names:
				if 'SST' in pre:
					...
					total += circuit_params["syn_cond"].at[pre, post]*circuit_params["n_cont"].at[pre, post]*circuit_params["conn_probs"].at[pre, post]
			circuit_params['SING_CELL_PARAM'].at['norm_tonic',post] -= circuit_params['SING_CELL_PARAM'].at['norm_tonic',post]*sst/total*MDD
			print(post, '_tonic reduced by: ', sst/total*100*MDD, '%') if RANK == 0 else None
